  ...
  const fn = (map) => {
    ...
    class InitScene {
      ...
      mounted() {
        ...
        // 入场动画
        runScene.cameraEx.setTemp("复原", { time: 2 });
      }
    ...
    class ClickEvents {
      ...
      refreshEnergyPosition(map) {
        ...
        const ps = { x, y };
        this.setPosition(doorDom, ps, true, { py: 100, px: -150 });
      }
      ...
      refreshTravelBuildPosition(map) {
        ...
        const { left: x, top: y } = core.events.get2DVec(map);
        const ps = { x, y };
        this.setPosition(eneryDom, ps, true, { py: 100, px: -150 });
      }
      ...
      refreshSafeDoorPosition(map) {
        ...
        const { left: x, top: y } = core.events.get2DVec(map);
        const ps = { x, y };
        this.setPosition(safeDom, ps, true, { py: 100, px: -150 });
      }
      ...
      refreshSafeOtherPosition(map) {
        ...
        const { left: x, top: y } = core.events.get2DVec(map);
        const ps = { x, y };
        this.setPosition(safeOtherDom, ps, true, { py: 100, px: -150 });
      }
      ...
      refreshComprehensivePosition(map) {
        ...
        const { left: x, top: y } = core.events.get2DVec(map);
        const ps = { x, y };
        this.setPosition(comprehensiveDom, ps, true, { py: 100, px: -150 });
      }
    ...
    class AreaPointModel {
      ...
      mounted() {
        ...
        Utils.getMacro(() => {
          if (path === 'energy') this.isShowDoor(false)
        }, 100)
      ...
      showParking(isShow) {
        ...
        core.addSprite.pointModel['doorName']['2号停车场'].sprite.visible = isShow;

        // 初始版本
        // this.parkModel['2号停车场'].visible = isShow;
      }
      ...
      isShowDoor(isShow) {
        ...
        // 初始版本
        return
        Object.keys(this.areaPointModel['大门点位']).map((name) => {
          const m = this.areaPointModel['大门点位'][name]
          if (name.includes('2号停车场')) return
          m.visible = isShow
        })

        Object.keys(this.areaPointModel['可点击大门点位']).map((name) => {
          const m = this.areaPointModel['可点击大门点位'][name]
          if (name.includes('2号停车场')) return
          m.visible = isShow
        })
      }
    ...
    class Camera {
      ...
      // 相机动画 重置视角
      resetCamera() {
        Utils.getMacro(() => runScene.cameraEx.setTemp('复原', { time: 1 }), 80);
      }

      // 相机动画
      cameraAnima(place) {
        console.log("触发的相机动画", place);
        new Promise((res) => {
    ...
    class Events {
      ...
      clickSprite(model) {
        ...
        // 综合态势 点击大门
        if (path === "overview") {
          core.clickEvents.clickComprehensiveModel(model);
          // 清楚上次点击的名称
          $send('clear-lastClickWorkShopName', '')
        }

        // 安防 点击模型
        if (path === "safe" || path === "travel") {
          core.clickEvents.clickSafeModel(model);
          // 清楚上次点击的名称
          $send('clear-lastClickWorkShopName', '')
        }
        // 便捷通行下 点击停车场
        if (path === "travel" && model.name === '2号停车场') {
          core.clickEvents.clickParking(model.name)
          // 清楚上次点击的名称
          $send('clear-lastClickWorkShopName', '')
        }
        // 能源下 点击停车场
        if (path === 'energy' && model.name === '2号停车场') {
          core.clickEvents.webClickEnergy(model.name);
          // 清楚上次点击的名称
          $send('clear-lastClickWorkShopName', '')
          core.closeFn.closeEnergySprite(model)
        }
